...
func main() {
	...
	r.POST("/updateDataFail", func(c *gin.Context) {
		...
		if err := updateDataFail(c); err != nil {
			fmt.Println("updateDataFail fail")
			c.JSON(http.StatusBadRequest, "updateData failure")
			return
		}
		fmt.Println("updateDataFail ok")
...
func insertOnUpdateDataFail(ctx *gin.Context) error {
...
func updateDataFail(ctx *gin.Context) error {
	...
	ret, err := db.ExecContext(ctx, sql, "test001", 3, 1)
	if err != nil {
		fmt.Printf("update failed, err:%v\n", err)
		return err
	}
	...
	if rows == 0 {
		return errors.New(fmt.Sprintf("update err %d", rows))
	}
	fmt.Printf("update success： %d.\n", rows)
